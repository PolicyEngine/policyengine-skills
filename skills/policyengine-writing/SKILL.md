...
description: |
  PolicyEngine writing style for any public-facing prose — blog posts, research reports,
  documentation, PR descriptions, social copy, READMEs. Enforces active voice, quantitative
  precision, sentence-case headings, and the analytical neutrality a nonpartisan 501(c)(3)
  requires: no policy prescriptions dressed as findings, no speculative claims, no one-sided
  framing, and explained counterintuitive results.
  Triggers: "write a blog post", "PolicyEngine blog", "policy analysis writeup", "PR description",
  "research report", "documentation", "social post", "changelog prose", "sounds like advocacy",
  "is this neutral", "sentence case", "writing style", "make it quantitative".
metadata:
  category: process
---
...
- **Example posts**: PolicyEngine's published articles live in policyengine-app-v2 at
  `app/src/data/posts/articles/` (metadata in `app/src/data/posts/posts.json`). Use the
  policyengine-research-lookup skill to find posts on a given topic to model tone against.
- **PolicyEngine site**: https://policyengine.org for linking to analyses and reforms.
- **Methodology**: state the model version and certified data bundle in every writeup — see
  the policyengine skill for how to read what you ran.
